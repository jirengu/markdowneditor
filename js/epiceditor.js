...
(function(window, undefined) {
  ...
  var AjaxFileUploader = function(success, error) {
    var _this = this;
    this.success = success || function() {};
    this.error = error || function() {};

    this.uploadFile = function(uploadUrl, form) {
  ...
  function EpicEditor(options) {
    ...
    var self = this,
      ...
      _defaultFileSchema, _defaultFile, defaults = {
        ...
        theme: {
          base: 'http://cdn.jirengu.com/editor/css/epiceditor.min.css',
          preview: 'http://cdn.jirengu.com/editor/css/markdown.min.css',
          editor: 'http://cdn.jirengu.com/editor/css/epic-dark.min.css'
        },
        focusOnLoad: true,
        shortcut: {
        ...
        uploadUrl: 'http://upload.hungerworks.com/upload'
      },
    ...
    self._uploadEnabled = typeof(buttons) === 'object' ? typeof buttons.upload === 'undefined' || buttons.upload : buttons === true;
  ...
  EpicEditor.prototype.load = function(callback) {
    ...
    _HtmlTemplates = {
      ...
      chrome: '<div id="epiceditor-wrapper" class="epiceditor-edit-mode">' +
        ...
        (self._uploadEnabled ? '<button title="' + this.settings.string.upload + '" class="epiceditor-upload-btn"></button>' : '') +
    ...
    utilBtns.addEventListener('click', function(e) {
      ...
      if (targetClass.indexOf('epiceditor-upload-btn') > -1) {
        self.upload();
      }
    ...
    this.input = this.createForm(this.settings.uploadUrl);
  ...
  /**
   * Will upload images and set content to editor
   * @returns {object} EpicEditor will be returned
   */
  EpicEditor.prototype.upload = function() {
    var self = this;
    console.log('upload')
    this.input.click();
  }

  EpicEditor.prototype.createForm = function(url) {
    ...
    var uploader = new AjaxFileUploader(function(ret) {
      if (ret && ret.status === 'success') {
        var str = '![](' + ret.imgUrl + ')';
        if (_isIE() > -1) {
          self.editorIframeDocument.selection.createRange().pasteHTML(str);
        } else {
          self.editorIframeDocument.execCommand("insertText", false, str);
        }
      } else {
        console.log('上传失败')
      }
